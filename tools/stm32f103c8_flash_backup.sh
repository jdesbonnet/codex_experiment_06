...
usage() {
  cat <<'EOF'
usage: tools/stm32f103c8_flash_backup.sh [options]

Dump the STM32F103C8 application flash through the CMSIS-DAP debugprobe.

Options:
  --backup-dir <dir>      Directory for backup artifacts.
  --output <file>         Explicit output image path.
  --flash-start <addr>    Flash base address. Default: 0x08000000
  --flash-size <bytes>    Bytes to back up. Default: 65536
...
PROJECT_CFG=${PROJECT_CFG:-targets/stm32f103c8/openocd/base.cfg}

FLASH_START=${FLASH_START:-0x08000000}
FLASH_SIZE_BYTES=${FLASH_SIZE_BYTES:-65536}
BACKUP_DIR=${BACKUP_DIR:-./backups/stm32f103c8}
...
if [ -z "${BACKUP_FILE}" ]; then
  BACKUP_FILE="${BACKUP_DIR}/stm32f103c8-flash-${TIMESTAMP_UTC}.bin"
fi
...
{
  echo "[${TIMESTAMP_UTC}] stm32f103c8_flash_backup.sh starting"
...
if ! "${OPENOCD_BIN}" "${OPENOCD_ARGS[@]}" 2>&1 | tee -a "${LOG_FILE}"; then
  echo "OpenOCD failed while backing up the STM32F103C8 flash. Check ${LOG_FILE}." >&2
  exit 1
fi
