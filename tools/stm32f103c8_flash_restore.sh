...
usage() {
  cat <<'EOF'
usage: tools/stm32f103c8_flash_restore.sh <backup.bin> [options]

Restore an STM32F103C8 application flash image through the CMSIS-DAP debugprobe.
...
LOG_FILE="${RESTORE_DIR}/stm32f103c8-restore-${TIMESTAMP_UTC}.log"
...
{
  echo "[${TIMESTAMP_UTC}] stm32f103c8_flash_restore.sh starting"
...
if ! "${OPENOCD_BIN}" "${OPENOCD_ARGS[@]}" 2>&1 | tee -a "${LOG_FILE}"; then
  echo "OpenOCD failed while restoring the STM32F103C8 flash. Check ${LOG_FILE}." >&2
  exit 1
fi
