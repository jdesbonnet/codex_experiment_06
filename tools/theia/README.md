...
| Host-side sim    | `tools/theia/sim/`                | shipped       |
| DAP server       | `tools/theia/dap/`                | shipped       |
| Theia IDE        | `tools/theia/theia/`              | shipped       |
| Helper scripts   | `tools/theia/scripts/`            | shipped       |
...
./tools/theia/sim/cli.py /tmp/count10.bin

# Or attach a debugger via the DAP server
python3 tools/theia/dap/server.py            # auto port, TCP
python3 tools/theia/dap/server.py --stdio    # stdio (Theia/VS Code mode)
```
...
`tools/theia/sim/test_sim.py`).

## Quick start (IDE)

```sh
tools/theia/scripts/install.sh    # one-time: node deps + theia build
...
2. Launches `python3 tools/theia/dap/server.py --stdio` as the debug
   adapter.
...
python3 tools/theia/sim/test_sim.py

# DAP server end-to-end tests (spawns the server, drives it over TCP)
python3 tools/theia/dap/test_dap.py
...
`tools/theia/theia/e2e/blink-debug.spec.ts` drives the actual browser
...
anything under `tools/theia/` or to `tools/vm_cc.py` / `tools/vm_asm.py`.
...
`tools/theia/dap/handlers.py`: adding a `HardwareBackend` later does not
...
- DAP session never stops: confirm `python3 tools/theia/dap/test_dap.py`
