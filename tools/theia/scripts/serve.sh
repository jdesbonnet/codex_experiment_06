...
#
# Usage:
#   tools/theia/scripts/serve.sh            # listens on 0.0.0.0:3000
#   PORT=3001 tools/theia/scripts/serve.sh
...
if [ ! -d "$THEIA_DIR/node_modules" ]; then
    echo "[serve] tools/theia/theia/node_modules missing. Run scripts/install.sh first." >&2
    exit 1
fi
