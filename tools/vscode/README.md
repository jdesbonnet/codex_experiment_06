...
Browser-only port of the development environment in `tools/theia/` (Theia),
...
| Path        | Purpose                                                                                  |
| ----------- | ---------------------------------------------------------------------------------------- |
| `sim/`      | wasm tiny_vm simulator (Rust → wasm-pack). Bit-exact mirror of `tools/theia/sim/`.    |
...
tools/vscode/scripts/serve.sh      # serves on http://localhost:3000/ + compile API on :3001
```
...
`tools/vscode/host/openapi.yaml`).
...
1. `cargo test` in `sim/rust/` — 11 unit + 8 regression tests asserting
   bit-for-bit parity with `tools/theia/sim/tiny_vm_sim.py` and the
...
```sh
(cd tools/vscode/sim/rust && cargo test)          # sim only
(cd tools/vscode/extension && npm run typecheck)  # extension only
(cd tools/vscode/e2e && npx playwright test)      # e2e only (needs serve.sh up or webServer auto-start)
```
...
```sh
tools/vscode/scripts/console-capture.sh > /tmp/cc.jsonl
...
Contrast with the Theia setup (`tools/theia/theia/`): there, the
