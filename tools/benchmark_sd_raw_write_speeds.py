#!/usr/bin/env python3
"""
Sweep the STM32F103C8 raw SD write benchmark across multiple SPI prescalers.

The raw benchmark writes directly into the preallocated contiguous CODXLOG.BIN
region and reports both single-block and multi-block throughput.
"""
...
BENCH_DIR = ROOT / "projects" / "sd_raw_write_bench" / "stm32f103c8_c"
BENCH_IMAGE = BENCH_DIR / "sd_raw_write_bench.elf"
READ_CHUNK = 256
BAUD = termios.B57600
MODES = ("single", "multi")
...
def parse_mode_result(mode: str, divider: int, uart_text: str) -> BenchResult:
    ...
    sysclk_info = match_ints(r"sd_raw_write_bench: sysclk_hz=(\d+)")
    mode_info = match_ints(rf"rawbench: mode={mode} .* spi_prescaler=/(\d+) spi_hz=(\d+)")
    time_info = match_ints(rf"rawbench: mode={mode} write_ms=(\d+) verify_ms=(\d+) total_ms=(\d+)")
    rate_info = match_floats(rf"rawbench: mode={mode} write_rate_kBps=([0-9]+\.[0-9]) total_rate_kBps=([0-9]+\.[0-9])")
    verify_info = re.search(rf"rawbench: mode={mode} verify_first=(OK|FAIL) verify_last=(OK|FAIL)", uart_text)

    ok = (
        "sd_raw_write_bench: complete" in uart_text
        and sysclk_info is not None
        and mode_info is not None
        and time_info is not None
        and rate_info is not None
        and verify_info is not None
        and verify_info.group(1) == "OK"
        and verify_info.group(2) == "OK"
    )
    ...
    if not ok:
        ...
        return BenchResult(
            ...
            spi_hz=mode_info[1] if mode_info is not None else None,
            write_ms=time_info[0] if time_info is not None else None,
            verify_ms=time_info[1] if time_info is not None else None,
            total_ms=time_info[2] if time_info is not None else None,
            write_rate_kbps=rate_info[0] if rate_info is not None else None,
            total_rate_kbps=rate_info[1] if rate_info is not None else None,
            ok=False,
            detail=tail,
        )
    ...
    return BenchResult(
        mode=mode,
        divider=divider,
        sysclk_hz=sysclk_info[0],
        spi_hz=mode_info[1],
        write_ms=time_info[0],
        verify_ms=time_info[1],
...
def run_one(divider: int, fd: int, timeout_s: float, verbose: bool, sysclk_hz: int, total_bytes: int) -> list[BenchResult]:
    flush_uart(fd)
    env = dict(os.environ)
    ...
    if build.returncode != 0:
        ...
        return [BenchResult(mode, divider, None, None, None, None, None, None, None, False, detail) for mode in MODES]
    ...
    if flash.returncode != 0:
        detail = flash.stderr.strip().splitlines()[-1] if flash.stderr.strip() else "flash failed"
        return [BenchResult(mode, divider, None, None, None, None, None, None, None, False, detail) for mode in MODES]

    uart_text = capture_until(fd, timeout_s, "sd_raw_write_bench: complete")
    if verbose:
        print(f"--- divider /{divider} UART ---")
        print(uart_text, end="" if uart_text.endswith("\n") else "\n")

    return [parse_mode_result(mode, divider, uart_text) for mode in MODES]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=64000000,
        help="MCU system clock for the benchmark build (default: 64000000).",
    )
    parser.add_argument(
        "--bytes",
        type=int,
        default=262144,
        help="Benchmark byte count per mode (default: 262144).",
    )
    parser.add_argument(
        "--timeout",
        type=float,
        default=90.0,
        help="UART capture timeout per divider in seconds (default: 90).",
    )
    parser.add_argument(
        "--uart-port",
...
def main() -> int:
    ...
    try:
        ...
        try:
            ...
            print("mode,divider,sysclk_hz,spi_hz,write_ms,verify_ms,total_ms,write_rate_kBps,total_rate_kBps,status")
            overall_ok = True
            for divider in dividers:
                results = run_one(divider, fd, args.timeout, args.verbose, args.sysclk_hz, args.bytes)
                for result in results:
                    ...
                    verify_ms = "" if result.verify_ms is None else str(result.verify_ms)
                    ...
                    print(
                        f"{result.mode},{result.divider},{sysclk_hz},{spi_hz},"
                        f"{write_ms},{verify_ms},{total_ms},{write_rate},{total_rate},{status}"
                    )
